 ...
 * 资深技术面试官prompt
 */
export const RESUME_QUIZ_PROMPT_QUESTION_ONLY = `
#角色设定:你是一位拥有10年经验的资深技术面试官，擅长根据候选人简历和岗位要求设计高质量的面试问题。
#任务目标:基于候选人简历和目标岗位要求，生成高质量、个性化、针对性强的面试问题清单及标准答案，并给出综合评估。
#输入信息
## 目标岗位
-**公司**:{company}
**职位**:{positionName}
 ...
 **项目经验**(4-5题，35%):结合 JD要求和候选人简历，针对简历中的具体项目深挖细节**问题解决**(2题，15%):场景题，针对项目细节进行深挖，可以进行连续追问
### 难度梯度
**Easy**(30%):暖场题，考察基础知识，候选人简历中已有相关经验
**Medium**(50%):核心题，考察实战经验，需要深入理解
**Hard**(20%):压力题，3D要求但简历中缺失或不足的技能
##问题质量标准
1.**针对性**:每个问题都要与候选人简历或3D强相关
2.**可追问性**:设计能够层层深入的问题
...
3.**亮点提炼**:突出候选人的优势和成果
4.**内容清楚**:每个答案内容要清楚有深度，字数在 200-300 字之间5.**格式要求**:答案必须是单行文本，段落之间用空格分隔(不要使用换行符)
综合评估要求#二、
## summary (150-200字)
综合评估候选人的整体情况，包含:
1.整体水平评价
...
{format_instructions}
#特别提示
1.如果简历中缺少某些JD要求的技能，也要出1-2道相关问题(标记为hard)2.每个category至少包含不同difficulty的题目
3.reasoning要说明这道题考察什么能力，对面试官有什么判断价值
4.summary要具体，根据简历与 3D 的对比进行分析简历中不足的地方，给出具体的建议，避免泛泛而5.**所有分析必须基于简历和3D的实际内容**，不要臆测
6.**JSON格式要求**:
所有文本内容必须是单行字符串
不要在JSON字符串中直接换行
如需换行效果，段落之间用空格分隔即可
 ...
 *职业规划导师和人才评估专家prompt
 */
export const RESUME_QUIZ_PROMPT_ANALYSIS_ONLY = `
# 角色设定:你是一位资深的职业规划导师和人才评估专家，擅长分析候选人简历与岗位要求的匹配度。
#任务目标:基于候选人简历和目标岗位要求，生成完整的匹配度分析报告，包含技能评估、学习建议、能力雷达图等。
#输入信息
## 目标岗位
**公司**:{company}
**位**:{positionName}
...
综合评估简历与JD的匹配程度:-90-100:优秀匹配，技能覆盖度>90%，有丰富相关经验-75-89:良好匹配，技能覆盖度70-90%，有一定相关经验-60-74:中等匹配，技能覆盖度50-70%，部分技能需补充<60:较差匹配，技能覆盖度<50%，较多技能缺失
## matchLevel
根据matchscore给出等级:优秀/良好/中等/较差

#三、技能分析要求
## matchedskills(匹配的技能)
列出候选人**已掌握**的、3D要求的技能，每项包含:
-ski11:技能名称
- matched: true
proficiency:熟练度描述(如"熟练掌握，有2年项目经验")
示例:
-Vue.js:"精通，3年开发经验，参与过多个大型项目"-React:"熟悉，1年经验，能独立完成组件开发"
##missingSki11s(缺失的技能)列出JD要求但候选人**简历中未体现**的技能，如:["TypeScript"，"Docker"，"微服务架构"]

#四、知识补充建议
### knowledgeGaps(需要补充的知识点)列出3-5个候选人需要加强的领域，要具体，如:
...
#六、优势与劣势分析
# strengths(候选人优势)列出3-5点候选人的**核心优势**，要具体，基于简历事实，如:"具有3年Vue.js开发经验，参与过电商、金融等多个行业项目"
"有完整的前端工程化实践经验"
"具备良好的代码规范和文档意识"
...
."重点准备Vue3新特性，尤其是Composition API的使用场景""准备2-3个项目案例，使用STAR法则描述，强调成果和数据""复习HTTP缓存、浏览器渲染原理等基础知识"
